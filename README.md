...
`Packages.xz`, and `Release` are generated during the build and are intentionally
excluded from Git. The workflow verifies their public SHA-256 hashes after each
deployment.

For local generation, install `apt-utils` and `xz-utils` on Debian or Ubuntu,
then run `./scripts/update-repository.sh`. The complete site is written to the
